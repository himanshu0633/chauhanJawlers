...
//     const filtered = categories.filter(item => item.variety === selectedVariety);
...
//     setImagePreview(item.image ? publicUrl(item.image) : null);
//     setFilteredCategories(filtered);
...
const SubCategoryJewel = () => {
  ...
  const startEditingSubCategory = (item) => {
    ...
    setFormData({
      name: item.name,
      description: item.description,
      image: null, // Keep this null, only set when user selects new file
      category_id: item.category_id?._id || '',
      subCategoryvariety: selectedVariety,
    });
    setImagePreview(item.image ? publicUrl(item.image) : null);
  ...
  return (
    <Container maxWidth="lg">
      <Box sx={{ my: 4 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'end', alignItems: 'center', my: 3 }}>
        ...
        {loading ? (
          ...
          <StyledTableContainer component={Paper}>
            ...
            <TablePagination
              ...
              onPageChange={handleChangePage}
              onRowsPerPageChange={handleChangeRowsPerPage}
              sx={{ backgroundColor: '#f5f5f5', borderBottomLeftRadius: '8px', borderBottomRightRadius: '8px' }}
            />
          </StyledTableContainer>
        )}
      </Box>
